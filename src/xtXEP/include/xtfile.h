 ...
 * The file must be closed with the commit flag set to commit the contents to flash. 
 *
 * @return  Status of execution as defined in @ref xtfile_error_codes_t
 */
...
/**
 * Gets the pointer and size of the buffered content of the file.
 * The content of the buffer can be modified while the file remains open, 
 * and will be committed to flash when the file is closed with the commit flag set.
...
uint32_t xtfile_get_buffer(
                        ...
                        void** data,                ///< Pointer to where to store the location of the buffer
                        uint32_t* size              ///< Pointer to variable to hold size of file
                        );

/**
 * Closes an open file. To commit contents in the file buffer to flash, set commit to true.
 *
 * @return  Status of execution as defined in @ref xtfile_error_codes_t
 */            
uint32_t xtfile_close(
                        void* handle,               ///< Handle of open file to close
                        bool commit                 ///< Commits the file to flash if this is true
                        );
